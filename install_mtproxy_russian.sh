...
update_mtproxy_secret() {
    ...
    if sed -i "s|-S [a-f0-9]\{32\}|-S ${NEW_SECRET}|" "$service_file"; then
        ...
        if systemctl restart mtproxy; then
            ...
            print_error "Проверьте логи: journalctl -u mtproxy"
            exit 1
        fi
    else
...
uninstall_mtproxy() {
    ...
    setcap -r /usr/local/bin/mtproto-proxy 2>/dev/null || true # Очистка capabilities
    ...
    if id "mtproxy" &>/dev/null; then
        userdel mtproxy 2>/dev/null || print_warning "Не удалось удалить пользователя 'mtproxy'. Возможно, он все еще владеет файлами."
    fi
    ...
    if grep -q "^net.core.somaxconn[[:space:]]*=.*1024" "$SYSCTL_FILE"; then
         ...
         sysctl -p || print_warning "Не удалось применить изменения sysctl."
    fi
...
curl -s https://core.telegram.org/getProxySecret -o /etc/mtproxy/proxy-secret || print_warning "Не удалось скачать proxy-secret."
curl -s https://core.telegram.org/getProxyConfig -o /etc/mtproxy/proxy-multi.conf || print_warning "Не удалось скачать proxy-multi.conf."
...
if command -v ufw &> /dev/null; then
    ...
    ufw allow $EXTERNAL_PORT/tcp comment "MTProxy External Port" || print_warning "Не удалось добавить правило UFW для порта $EXTERNAL_PORT."
    print_status "Правило UFW добавлено для внешнего порта $EXTERNAL_PORT/tcp."
    print_warning "Если UFW выключен, включите его: ufw enable"
...
if systemctl start mtproxy; then
    ...
    print_error "Не удалось запустить сервис MTProxy. Проверьте логи: journalctl -u mtproxy -f"
    exit 1
fi
...
sysctl -p || print_warning "Не удалось применить sysctl -p."
